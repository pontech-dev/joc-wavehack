...
import { chain, nftAddress } from "@/app/data";
...
export default function Example() {
  ...
  const handleClick = async () => {
    const response = await fetch("/products/1/items/1");
    const blob = await response.blob();
    console.log(blob);
    const url = window.URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = "image1ss11.png";
    link.click();
  ...
  const { data, isLoading, isError } = useWalletBalance({
    chain: { id: 10081, rpc: "https://rpc-1.testnet.japanopenchain.org:8545" },
    address: activeAccount?.address,
    client,
    tokenAddress: "0x9f5bFA5D2C903a59fAd17b71D346484f992caa21",
  });
  ...
  const [hasNFT, setHasNNFT] = useState(false);
